...
import { products, articles, faqItems, galleryImages, customPages, siteSettings } from "@db/schema";
...
export const contentRouter = createRouter({
  ...
  productGet: publicQuery.input(z.object({ id: z.number() })).query(async ({ input }) => {
    const db = getDb();
  ...
  articleGet: publicQuery.input(z.object({ id: z.number() })).query(async ({ input }) => {
    const db = getDb();
    const rows = await db.select().from(articles).where(eq(articles.id, input.id));
    return rows[0] ?? null;
  }),
  ...
  faqUpdate: publicQuery
    .input(z.object({ id: z.number(), question: z.string().min(1), answer: z.string().min(1) }))
    .mutation(async ({ input }) => {
      const { id, ...data } = input;
      const db = getDb();
  ...
  pageGet: publicQuery.input(z.object({ slug: z.string() })).query(async ({ input }) => {
    const db = getDb();
    const rows = await db.select().from(customPages).where(eq(customPages.slug, input.slug));
    return rows[0] ?? null;
  }),
  pageCreate: publicQuery
    .input(z.object({ title: z.string().min(1), slug: z.string().min(1), content: z.string() }))
    .mutation(async ({ input }) => {
      const db = getDb();
      const result = await db.insert(customPages).values(input);
      return { id: Number(result[0].insertId) };
    }),
  pageUpdate: publicQuery
    .input(z.object({ id: z.number(), title: z.string().min(1), slug: z.string().min(1), content: z.string() }))
    .mutation(async ({ input }) => {
      const { id, ...data } = input;
      const db = getDb();
  ...
  settingsGet: publicQuery.input(z.object({ key: z.string() })).query(async ({ input }) => {
    const db = getDb();
